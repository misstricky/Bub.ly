...
class FunctionalityTest(TestCase):
    ...
    def test_register(self):
        ...
        response = self.client.post('/_register/', {'email': 'some@something.com', 'password': '123456'}, follow=True)
        self.assertEquals(response.status_code, 200)
        self.assertEquals(response.redirect_chain, [('http://testserver/', 302)])
        self.assertEquals(True, redis_ob.exists("user:email:%s" %md5_constructor('some@something.com').hexdigest()))
        # cant assert the user:1 because redis might already have a user:1
        # and we dont clear redis data 

    def test_login(self):
        response = self.client.post('/_login/', {'email': 'some@something.com', 'password': '123456'}, follow=True)
        self.assertEquals(response.status_code, 200)
